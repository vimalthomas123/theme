...
<?php
      if( have_rows('content_section_quick_facts') ):
            while ( have_rows('content_section_quick_facts') ) : the_row();
                  if( get_row_layout() == 'the_vision' ):
                        ...
                        <section class="vision__banner" style="--bg-color: <?php echo $color;?>">
                              <div class="container">
                                    ...
                                    <div class="vision__banner__content">
                                          ...
                                          ?>
                                                      <a class="cta__primary" href="<?php echo esc_url( $cta_url ); ?>" target="<?php echo esc_attr( $cta_target ); ?>"><?php echo esc_html( $cta_title ); ?></a>
                                          <?php endif; ?>
                                    </div>
                              </div>
                        </section>
<?php 
                  elseif( get_row_layout() == 'global_presence' ):
                        $title = get_sub_field('title');
                        $color = get_sub_field('color');
?>
                        <section class="enduring__values" style="--bg-color: <?php echo $color;?>">
                              <?php if($title):echo'<div class="container">
                                    <h2 class="title__secondary text-center text-white">'.$title.'</h2>
                              </div>'; endif;?>
                        </section>
                        <section class="sales__column__grid">
                              <div class="row">
                                    <?php
                                          if(have_rows('global_presence_repeter')):
                                                while(have_rows('global_presence_repeter')):the_row();
                                                      $icon = get_sub_field('icon');
                                                      $count = get_sub_field('count');
                                                      $description = get_sub_field('description');
                                                      $color = get_sub_field('color');
                                                      $count_postfix = get_sub_field('count_postfix');
                                    ?>
                                                      <div class="col-12 col-md-4 col-xl-4">
                                                            <div class="sales__column__grid__card" style="--bg-color: <?php echo $color; ?>">
                                                                  <?php if($icon): echo'<div class="sales__column__grid__card__icon">
                                                                        <img src="'.$icon.'" alt="'.$description.'">
                                                                  </div>'; endif;?>
                                                                  <div class="sales__column__grid__card__body">
                                                                        <h3>
                                                                              <span class="counters" data-number="<?php echo $count;?>"><?php echo $count;?></span><?php echo $count_postfix;?>
                                                                        </h3>
                                                                        <?php if($description): echo'<h6>'.$description.'</h6>'; endif;?>
                                                                  </div>
                                                            </div>
                                                      </div>
                                    <?php 
...
<?php 
                  elseif( get_row_layout() == 'leading_positions' ):
                        $main_title = get_sub_field('main_title');
                        $sub_title = get_sub_field('sub_title');
                        $color = get_sub_field('color');
                        $manage_image_size = get_sub_field('manage_image_size');
?>
                        <section class="enduring__values enduring__values--light" style="--bg-color:<?php echo $color;?>">
                              <div class="container">
                                    <?php if($main_title): echo'<h2 class="title__secondary text-center text-white">'.$main_title.'</h2>'; endif;?>
                                    <?php if($sub_title): echo'<h5 class="sub__title text-center text-white mb-0">'.$sub_title.'</h5>'; endif;?>
                              </div>
                        </section>
                        <section class="leading__positions__grid">
                              <div class="row">
                                    <?php 
                                          if(have_rows('leading_positions_repeater')):
                                                while(have_rows('leading_positions_repeater')):the_row();
                                                      $title = get_sub_field('title');
                                                      $logo_img = get_sub_field('logo');
                                                      $logo = $logo_img['url'];
                                    ?>
                                                      <div class="col-12 col-md-4 col-xl-4">
                                                            <div class="leading__positions__grid__card">
                                                                  <?php if($title): echo'<h3>'.$title.'</h3>'; endif;?>
                                                                  <div class="leading__positions__grid__card__image <?php if($manage_image_size){ echo 'leading__positions__grid__card__image--icon';}?>">
                                                                        <?php if($logo): echo'<img src="'.$logo.'" alt="'.$logo_img['alt'].'">'; endif;?>
                                                                  </div>
                                                            </div>
                                                      </div>
                                    <?php
...
<?php 
                  elseif( get_row_layout() == 'top_retail_brands_group' ):
                        $main_title_ = get_sub_field('main_title_');
                        $sub_title = get_sub_field('sub_title');
                        $color = get_sub_field('color');
?>
                        <section class="enduring__values enduring__values--light" style="--bg-color: <?php echo $color;?>">
                              <div class="container">
                                    <?php if($main_title_): echo'<h2 class="title__secondary text-center text-white">'.$main_title_.'</h2>'; endif;?>
                                    <?php if($sub_title): echo'<h5 class="sub__title text-center text-white mb-0">'.$sub_title.'</h5>'; endif;?>
                              </div>
                        </section>
                        <section class="leading__positions__grid leading__positions__grid--reverse">
                              <div class="row">
                                    <?php 
                                          if(have_rows('brands')):
                                    ...
                                    ?>
                                                      <div class="col-12 col-md-4 col-xl-4">
                                                            <div class="leading__positions__grid__card d-flex justify-content-center align-items-center"><!-- <h3>#1 in the UaE& KSE</h3> -->
                                                                  <?php if($image): echo'<div class="leading__positions__grid__card__image">
                                                                        <img src="'.$image.'" alt="'.$image_img['alt'].'">
                                                                  </div>'; endif;?>
                                                            </div>
                                                      </div>
