...
<?php
if (have_rows('content_section_achievements')) :
    while (have_rows('content_section_achievements')) : the_row();
        if (get_row_layout() == 'text_&_image_group') :
            ...
            $image = $image_img['url'];
?>
            <section class="vision__banner" style="--bg-color: <?php echo $bg_color; ?>">
                <div class="container">
        ...
        elseif (get_row_layout() == 'content_section') :
            $description = get_sub_field('description');
            $more__description = get_sub_field('more_description');
        ...
        ?>
            <section class="manufacturing__certification">
                <div class="container">
                    ...
                    <?php if ($more__description) : ?>
                        <div class="more__info__block" style="display: none;">

                            <?php echo $more__description; ?>
                        </div>
                    <?php endif; ?>
                    ...
                    ?>
                        <button id="read__more__btn" class="cta__primary mx-auto"
        ...
        ?>
            <section class="manufacturing__certification pt_80">
                <div class="container">
                    <div class="achivement__grid__wrap row">
                        <div class="col-12 col-lg-6">
                            ...
                            if (have_rows('grid')) :
                                while (have_rows('grid')) : the_row();
                                    ...
                                    $title_postfix = get_sub_field('title_postfix');
                                    $sub_title = get_sub_field('sub_title');
                                    $sub_description = get_sub_field('sub_description');
                                    $color = get_sub_field('color');
                            ?>
                                    <div class="achivement__grid__card" style="--text-color: <?php echo $color; ?>">
                                        <?php if ($image) : echo '<div class="achivement__grid__card__icon">
                                                                        <img src="' . $image . '" alt="' . $image_img['alt'] . '">
                                                                  </div>';
                                        endif; ?>
                                        <?php if ($title || $counter) : echo '<h4>' . $title . '<span class="counters" data-number="' . $counter . '">' . $counter . '</span>' . $title_postfix . '</h4>';
                                        endif; ?>
                                        <?php if ($sub_title) : echo '<h5>' . $sub_title . '</h5>';
                                        endif; ?>
                                        <?php if ($sub_description) : echo '<p>' . $sub_description . '</p>';
                                        endif; ?>
                                    </div>
                            <?php
                                endwhile;
                        ...
                        <div class="col-12 col-lg-6">
                            <?php
                            if (have_rows('grid_group')) :
                                while (have_rows('grid_group')) : the_row();
                                    $icon_img = get_sub_field('icon');
                                    $icon = $icon_img['url'];
                                    $text = get_sub_field('text');
                            ?>
                                    <div class="achivement__grid__card achivement__grid__card--lg achivement__grid__card--header" style="--text-color: #000">
                                        <?php if ($icon) : echo '<div class="achivement__grid__card__icon">
                                                                        <img src="' . $icon . '" alt="' . $icon_img['alt'] . '">
                                                                  </div>';
                                        endif; ?>
                                        <?php if ($text) : echo '<h5>' . $text . '</h5>';
                                        endif; ?>
                                    </div>
                            <?php
                                endwhile;
                            ...
                            ?>
                            <?php
                            if (have_rows('achieve')) :
                                while (have_rows('achieve')) : the_row();
                                    $color = get_sub_field('color');
                                    $counter = get_sub_field('counter');
                                    $title_postfix = get_sub_field('title_postfix');
                                    ...
                                    $chart = 100 - $counter;
                            ?>
                                    <div class="achivement__grid__card mt-0" style="--text-color: <?php echo $color; ?>">
                                        <div class="achivement__grid__card__graph">
                                            <div class="my__chart__wrap donut__chart mx-auto no__filter" data-colors="<?php echo $color; ?>" style="--color: <?php echo $color; ?>" data-values="<?php echo $counter, ',', $chart; ?>">
                                                <div class="myChart__inner">
                                                    <canvas class="myChart donut"></canvas>
                                                    <div class="my__chart__value" data-number="<?php echo $counter; ?>"></div>
                                                </div>
                                            </div>
                                        </div>
                                        <h5>
                                            <strong><?php echo $title_text; ?> <span class="counters" data-number="<?php echo $counter; ?>"></span><?php echo $title_postfix; ?></strong>
                                        </h5>
                                        <?php if ($description) : echo '<p>' . $description . '</p>';
                                        endif; ?>
                                    </div>
                    ...
                    <?php if ($file) : echo '<a href="' . $file . '" arget="_blank" class="cta__primary mx-auto">' . $cta_title . '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M10.586 5.657L6.636 1.707C6.45384 1.51839 6.35305 1.26579 6.35533 1.00359C6.3576 0.741397 6.46277 0.490585 6.64818 0.305177C6.83359 0.119768 7.0844 0.0145995 7.3466 0.0123211C7.6088 0.0100427 7.8614 0.110837 8.05 0.292995L13.707 5.95C13.8002 6.04265 13.8741 6.15281 13.9246 6.27414C13.9751 6.39548 14.001 6.52559 14.001 6.657C14.001 6.7884 13.9751 6.91852 13.9246 7.03985C13.8741 7.16118 13.8002 7.27134 13.707 7.364L8.05 13.021C7.95775 13.1165 7.84741 13.1927 7.7254 13.2451C7.6034 13.2975 7.47218 13.3251 7.3394 13.3262C7.20662 13.3274 7.07494 13.3021 6.95205 13.2518C6.82915 13.2015 6.7175 13.1273 6.62361 13.0334C6.52971 12.9395 6.45546 12.8278 6.40518 12.7049C6.3549 12.5821 6.3296 12.4504 6.33075 12.3176C6.3319 12.1848 6.35949 12.0536 6.4119 11.9316C6.46431 11.8096 6.54049 11.6992 6.636 11.607L10.586 7.657H1C0.734784 7.657 0.48043 7.55164 0.292893 7.3641C0.105357 7.17657 0 6.92221 0 6.657C0 6.39178 0.105357 6.13742 0.292893 5.94989C0.48043 5.76235 0.734784 5.657 1 5.657H10.586Z" fill="white" />
                                    </svg>
                              </a>';
                    endif; ?>
